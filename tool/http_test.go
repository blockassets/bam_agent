...
func TestHtmlHandlerFunc_ServeHTTP(t *testing.T) {
	req := httptest.NewRequest("GET", "/doesnotmatter", nil)
	w := httptest.NewRecorder()

	var calledFunc = false

	fun := HtmlHandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calledFunc = true
	})
	fun.ServeHTTP(w, req)
...
func TestHtmlHandlerFunc_ServeHTTP_NoPurpose(t *testing.T) {
	req := httptest.NewRequest("GET", "/doesnotmatter", nil)
	req.Header.Set("X-Purpose", "Preview")
